        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <main class="lg:col-span-8">
                ...
                <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach(array_filter(explode(',', str_replace(['، ', ' '], ',', $article->meta_keywords))) as $keyword)
                            <a href="{{ route('front.search', ['q' => trim($keyword)]) }}" 
                ...
                @endif
                     ...
                     <div class="mt-12 sm:mt-16 pt-6 sm:pt-8 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <div class="flex flex-wrap gap-3 sm:gap-4">
                                <button onclick="copyToClipboard('{{ route('front.news.show', $article->id) }}')"
                                ...
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('front.news.show', $article->id)) }}" 
                                   target="_blank" 
                                ...
                                <a href="https://wa.me/?text={{ urlencode($article->title . ' ' . route('front.news.show', $article->id)) }}" 
                                   target="_blank" 
                                ...
                                <a href="https://twitter.com/intent/tweet?text={{ urlencode($article->title) }}&url={{ urlencode(route('front.news.show', $article->id)) }}" 
                                   target="_blank" 
                                ...
                                <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(route('front.news.show', $article->id)) }}&title={{ urlencode($article->title) }}" 
                                   target="_blank" 
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            @foreach($relatedArticles->take(4) as $related)
                                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden group hover:shadow-lg transition-all duration-300">
                                    <a href="{{ route('front.news', $related->id) }}" class="block">
                                        @if($related->image)
                                            <div class="relative h-40 overflow-hidden">
                                                <img src="{{ url('storage/' . $related->image) }}" 
                                                     alt="{{ $related->title }}" 
