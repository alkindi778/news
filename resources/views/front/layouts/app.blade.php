...
<head>
    ...
    <meta name="description" content="{{ $site_settings->description ?? config('app.description', 'موقع عدن الإخباري') }}">
    <meta name="keywords" content="{{ $site_settings->keywords ?? config('app.keywords', 'أخبار,عدن,اليمن') }}">
    
    <title>@yield('title') {{ $site_settings->site_name ? '| '.$site_settings->site_name : config('app.name') }}</title>
...
<body class="font-kufi bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-gray-100" x-data="{ mobileMenu: false }">
    ...
    @if($breaking_news = \App\Models\News::where('is_breaking', true)->where('status', 'published')->latest()->first())
    <div x-data="{ showBreakingNews: localStorage.getItem('hideBreakingNews_{{ $breaking_news->id }}') !== 'true' }" 
            ...
            <div class="container mx-auto px-6 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-6 space-x-reverse">
                        ...
                        <a href="{{ route('front.news.show', $breaking_news->slug) }}" class="text-white hover:text-red-100 font-bold text-xl">
                            {{ $breaking_news->title }}
                        </a>
                    </div>
                    <button @click="showBreakingNews = false; localStorage.setItem('hideBreakingNews_{{ $breaking_news->id }}', 'true')" 
    ...
    <script>
        document.addEventListener('copy', function(e) {
            ...
            if(selectedText && selectedText.trim() !== '') {
                ...
                additionalText += '\nصفحتنا على الفيس بوك {{ $site_settings->facebook_url }}';
                @endif
                
                @if($site_settings->twitter_url)
                additionalText += '\nصفحتنا على تويتر {{ $site_settings->twitter_url }}';
                @endif
