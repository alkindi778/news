...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="bg-white shadow-md rounded-lg p-6">
            ...
            <form action="{{ route('admin.sidebar.update', $section) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="type" id="type" required onchange="toggleFields()"
                        ...
                        <option value="category" {{ old('type', $section->type) == 'category' ? 'selected' : '' }}>تصنيف</option>
                        ...
                        <option value="ads" {{ old('type', $section->type) == 'ads' ? 'selected' : '' }}>إعلان</option>
                ...
                <div id="ad_field" class="hidden">
                    ...
                    <select name="ad_id" id="ad_id"
                        ...
                        @foreach($ads as $ad)
                            <option value="{{ $ad->id }}" 
                                {{ old('ad_id', $section->ad_id) == $ad->id ? 'selected' : '' }}>
                                {{ $ad->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div id="content_field" class="hidden">
                    ...
                    <textarea name="content" id="content" rows="4"
                              class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('content', $section->content) }}</textarea>
                </div>
                ...
                <div id="posts_count_field" class="hidden">
                    ...
                    <input type="number" name="posts_count" id="posts_count" 
                           value="{{ old('posts_count', $section->posts_count ?? 5) }}" min="1" max="50"
                           class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
