...
@section('title', 'تعديل غلاف صحيفة')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 py-6 px-4 sm:py-12 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
            ...
            <div class="relative h-auto sm:h-40 bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-500 dark:to-indigo-500 p-6 sm:p-0">
                ...
                <div class="relative sm:absolute inset-0 flex flex-col sm:flex-row items-center justify-between px-4 sm:px-8 py-6 sm:py-0">
                    <div class="text-center sm:text-left mb-4 sm:mb-0">
                        <h1 class="text-2xl sm:text-3xl font-bold text-white mb-2">تعديل غلاف صحيفة</h1>
                        <p class="text-white text-opacity-90">تعديل تفاصيل الغلاف</p>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
            <form action="{{ route('admin.newspaper-covers.update', $newspaperCover) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="group">
                        ...
                        <div class="mt-1 relative">
                            <input type="text" 
                                   name="title" 
                                   id="title" 
                                   value="{{ old('title', $newspaperCover->title) }}"
                    ...
                    <div class="group">
                        ...
                        <div class="mt-1 relative">
                            <input type="text" 
                                   name="newspaper_name" 
                                   id="newspaper_name" 
                                   value="{{ old('newspaper_name', $newspaperCover->newspaper_name) }}"
                                   class="block w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:focus:ring-blue-400 transition-all duration-200"
                ...
                <div class="group">
                    ...
                    <!-- Image Preview -->
                    <div id="imagePreview" class="{{ !$newspaperCover->cover_image ? 'hidden' : '' }} mt-2 relative">
                        <div class="relative w-full h-64 rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-700">
                            <img id="preview" src="{{ $newspaperCover->cover_image ? url('storage/' . $newspaperCover->cover_image) : '#' }}" 
                                 alt="{{ $newspaperCover->title }}" 
                    ...
                    <!-- Upload Area -->
                    <label id="uploadArea" class="{{ $newspaperCover->cover_image ? 'hidden' : '' }} relative flex flex-col items-center justify-center h-48 mt-2 rounded-xl border-3 border-dashed border-gray-300 dark:border-gray-600 hover:border-blue-500 dark:hover:border-blue-400 transition-colors duration-200 cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p class="mt-3 text-base text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400">
                                <span class="font-medium">اضغط لتغيير الصورة</span> أو اسحب وأفلت
                            </p>
                    ...
                    <div class="group">
                        ...
                        <div class="mt-1 relative">
                            <input type="url" 
                                   name="pdf_link" 
                                   id="pdf_link" 
                                   value="{{ old('pdf_link', $newspaperCover->pdf_link) }}"
                                   class="block w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:focus:ring-blue-400 transition-all duration-200"
                    ...
                    <div class="group">
                        ...
                        <div class="mt-1 relative">
                            <input type="date" 
                                   name="publish_date" 
                                   id="publish_date" 
                                   value="{{ old('publish_date', $newspaperCover->publish_date->format('Y-m-d')) }}"
                                   class="block w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:focus:ring-blue-400 transition-all duration-200">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="group">
                    ...
                    <div class="mt-1 relative">
                        <select name="status" 
                                id="status" 
                                class="block w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:focus:ring-blue-400 transition-all duration-200">
                            <option value="active" {{ old('status', $newspaperCover->status) == 'active' ? 'selected' : '' }}>نشط</option>
                            <option value="inactive" {{ old('status', $newspaperCover->status) == 'inactive' ? 'selected' : '' }}>غير نشط</option>
                        </select>
                    </div>
                    @error('status')
                        <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
