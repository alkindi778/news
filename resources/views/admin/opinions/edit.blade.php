...
@section('title', 'تعديل المقال - ' . $opinion->title)
...
<div class="p-2 sm:p-6">
    <div class="max-w-7xl mx-auto p-3 sm:p-6">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    <div class="text-center sm:text-right">
                        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">تعديل المقال</h1>
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $opinion->title }}</p>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden">
            <form action="{{ route('admin.opinions.update', $opinion) }}" method="POST" class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="author_id" 
                        ...
                        @foreach($authors as $author)
                            <option value="{{ $author->id }}" {{ old('author_id', $opinion->author_id) == $author->id ? 'selected' : '' }}>
                                {{ $author->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="content" 
                              ...
                              required
                              class="block w-full px-4 py-3 rounded-lg border-2 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 focus:border-blue-500 transition duration-200"
                    >{{ old('content', $opinion->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="status" 
                            ...
                            required
                            class="block w-full px-4 py-3 rounded-lg border-2 dark:border-gray-600 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 focus:border-blue-500 transition duration-200">
                        <option value="published" {{ old('status', $opinion->status) == 'published' ? 'selected' : '' }}>منشور</option>
                        <option value="draft" {{ old('status', $opinion->status) == 'draft' ? 'selected' : '' }}>مسودة</option>
                        <option value="pending" {{ old('status', $opinion->status) == 'pending' ? 'selected' : '' }}>معلق</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4">
                    <button type="button" 
                            ...
                            class="w-full sm:w-auto px-6 py-3 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg transition-colors duration-200">
                        إلغاء
                    </button>
                    <button type="submit"
                            class="w-full sm:w-auto px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors duration-200">
