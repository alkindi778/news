                    ...
                    @foreach($breakingNews as $item)
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 relative">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <form action="{{ route('admin.news.update', $item->id) }}" method="POST" class="flex">
                                    @csrf
                                    @method('PUT')
                            ...
                            <div class="text-sm text-gray-600 dark:text-gray-400">
                                <p><i class="far fa-clock ml-1"></i>{{ $item->created_at->diffForHumans() }}</p>
                                <p><i class="far fa-eye ml-1"></i>{{ $item->views_count }} مشاهدة</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                        ...
                        <div class="flex items-start space-x-4 space-x-reverse">
                            @if($newsItem->image)
                                <img class="w-20 h-20 rounded-lg object-cover" 
                                     src="{{ url('storage/' . $newsItem->image) }}" 
                                     alt="{{ $newsItem->title }}"
                                     onerror="this.onerror=null; this.src='{{ asset('images/placeholder.jpg') }}';">
                            @endif
                            <div class="flex-1">
                                <div class="font-semibold text-gray-900 dark:text-white mb-2">{{ $newsItem->title }}</div>
                                ...
                                <div class="flex flex-wrap gap-2 mb-2">
                                    @foreach($newsItem->categories as $category)
                                        <span class="bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-xs font-medium px-2 py-0.5 rounded">{{ $category->name }}</span>
                                    @endforeach
                                </div>
                            ...
                            <tr class="border-b border-gray-200 dark:border-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700/25">
                                <td class="px-4 py-3">
                                    <div class="flex items-center">
                                        ...
                                        @endif
                                        <div>
                                            <div class="font-semibold text-gray-900 dark:text-white">{{ $newsItem->title }}</div>
                                            @if($newsItem->featured)
                                ...
                                <td class="px-4 py-3">
                                    @foreach($newsItem->categories as $category)
                                        <span class="bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 text-xs font-medium ml-2 px-2.5 py-0.5 rounded">{{ $category->name }}</span>
                                    @endforeach
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">{{ number_format($newsItem->views_count) }}</td>
                                <td class="px-4 py-3">{{ $newsItem->published_at ? $newsItem->published_at->format('Y-m-d H:i') : '-' }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-3 space-x-reverse">
                                        <a href="{{ route('admin.news.edit', $newsItem) }}" class="text-blue-500 dark:text-blue-400 hover:text-blue-600">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.news.destroy', $newsItem) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا الخبر؟')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-500 dark:text-red-400 hover:text-red-600">
                                                <i class="fas fa-trash"></i>
...
<script>
    ...
    async function performSearch(query) {
        try {
            const response = await fetch(`{{ route('admin.news.search') }}?query=${encodeURIComponent(query)}`, {
                headers: {
