    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-gradient-to-r from-indigo-600 to-purple-700 rounded-t-3xl shadow-2xl p-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-4xl font-extrabold text-white leading-tight">إضافة قسم جديد</h1>
                    <p class="mt-2 text-indigo-100 text-lg">ابتكر قسمًا فريدًا لإثراء الشريط الجانبي</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-b-3xl shadow-2xl p-10">
            ...
            <form action="{{ route('admin.sidebars.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-8">
                        ...
                        <div class="relative">
                            <select name="type" id="type" 
                                ...
                                <option value="category" {{ old('type') == 'category' ? 'selected' : '' }}>قسم من الأقسام</option>
                                <option value="ads" {{ old('type') == 'ads' ? 'selected' : '' }}>إعلانات</option>
                                <option value="popular" {{ old('type') == 'popular' ? 'selected' : '' }}>الأكثر قراءة</option>
                                <option value="opinions" {{ old('type') == 'opinions' ? 'selected' : '' }}>مقالات الرأي</option>
                        ...
                        <div id="posts-count-field" class="relative hidden">
                            <input type="number" name="posts_count" id="posts_count" value="{{ old('posts_count', 5) }}"
                                   min="1" max="20"
                        ...
                        <div class="relative">
                            <input type="number" name="posts_count" id="posts_count" value="{{ old('posts_count', 6) }}" min="1" max="20"
                                   class="peer block w-full px-4 py-3 border-2 border-gray-300 rounded-xl dark:border-gray-600 dark:bg-gray-700 dark:text-white text-gray-900 focus:outline-none focus:border-indigo-500 placeholder-transparent transition-colors duration-300">
                        ...
                        <div class="relative">
                            <select name="layout_type" 
                                    class="block w-full px-4 py-3 border-2 border-gray-300 rounded-xl dark:border-gray-600 dark:bg-gray-700 dark:text-white text-gray-900 focus:outline-none focus:border-indigo-500 transition-colors duration-300 appearance-none">
                                <option value="grid" {{ old('layout_type') == 'grid' ? 'selected' : '' }}>شبكة</option>
                                <option value="list" {{ old('layout_type') == 'list' ? 'selected' : '' }}>قائمة</option>
                            </select>
                            <label class="absolute left-4 -top-5 text-sm text-gray-600 dark:text-gray-400">
                                نوع العرض
                            </label>
                    ...
                    <div id="ad-fields" class="space-y-8 hidden">
                        <div class="relative">
                            <select name="ad_id" 
                                    class="block w-full px-4 py-3 border-2 border-gray-300 rounded-xl dark:border-gray-600 dark:bg-gray-700 dark:text-white text-gray-900 focus:outline-none focus:border-indigo-500 transition-colors duration-300 appearance-none">
                    ...
                    <div id="custom-fields" class="space-y-8 hidden">
                        <div class="relative">
